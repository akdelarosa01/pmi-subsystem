                              ...
                              <div class="col-md-12">
                                   <form class="form-horizontal">
                                        ...
                                        <div class="form-group">
                                             <label for="inputname" class="col-md-3 control-label">*Quantity</label>
                                             <div class="col-md-9">
                                                  <input type="text" class="form-control input-sm clearbatch" id="edit_inputQty" placeholder="Quantity" name="qty" readonly />
     ...
     <script type="text/javascript">
          $(function() {
               ...
               $('#btn_edit_batch_modal').on('click', function() {
                    ...
                    $('#in_batch_qty'+bid).val($('#edit_inputQtyHidden').val());
               ...
               $('#uploadbatchfiles').on('submit', function(e){
                    ...
                    if ($("#batchfiles").val() == '') {
                         $('#loading').modal('hide');
                         failedMsg("Upload field is empty");
                    }
                    if (ext != 'xls' && ext != 'xlsx') {
                         $('#loading').modal('hide');
                         failedMsg("Please upload a valid excel file.");
                    } else {
                        if (fileName != ''){
                           if (ext == 'xls' || ext == 'xlsx') {
                                ...
                                $.ajax({
                                     ...
                                     if (datas.return_status == 'success') {
                                         getMRdata(datas.receivingno);
                                          successMsg("Items were successfully uploaded.");
                                     } else {
                                        $('#loading').modal('hide');
                                        if (datas.receivingno != '') {
                                            failedMsg("Please check this error.["+datas.receivingno+"]");
                                        }
                                        if (datas.item != '') {
                                            failedMsg("Please check this error.["+datas.item+"]");
                                        }
                                        if (datas.invoice != '') {
                                            failedMsg("Please check this error.["+datas.invoice+"]");
                                        }
                                     }
                                     $('#loading').modal('hide');
               ...
               $.each(summarydata, function(index, x) {
                    ...
                    table = '<tr class="summary_remove">'+
                                   ...
                                   '<td class="col-xs-1 text-center" id="td_iqc_chk_'+x.item+'">'+
                                        '<input type="checkbox" name="iqc[]" class="iqc_chk" id="in_iqc_chk_'+x.item+'" '+checked+' value="'+x.item+'" disabled="disabled">'+
                                   '</td>'+
                              '</tr>';
          ...
          function batching() {
              ...
              if (item == '' || qty == '' || box == '' || box_qty == '' || lot_no == '' || supplier == '') {
                ...
                //    r_qty = parseFloat($('#in_r_qty_'+item).val()) + parseFloat(qty);
                //    variance = parseFloat($('#in_var_'+item).val());
                //    new_var_qty = variance - qty;
                   //
                //    $('#td_r_qty_'+item).html(r_qty+'<input type="text" name="received_qty[]" id="in_r_qty_'+item+'"/>');
                //    $('#in_r_qty_'+item).val(r_qty);
                //    $('#td_var_'+item).html(new_var_qty+'<input type="text" name="variance[]" id="in_var_'+item+'"/>');
                //    $('#in_var_'+item).val(new_var_qty);
