...
function groupby(){
    ...
    var myData = {g1:g1,g2:g2,g3:g3,datefrom:datefrom,dateto:dateto};
	$('#tblforfgs').html("");
...
function loadFGSdata(){
    $.get("<?php echo e(url('/FGSgetrows')); ?>",
    {
        _token:$('meta[name=csrf-token]').attr('content')
    }).done(function(data, textStatus, jqXHR){
        console.log(data);
...
function FGSgetDataTable(data) {
    ...
    $.each(data,function(i,val){
        ...
        var tblrow = '<tr>'+
                '<td>'+
                    '<input type="checkbox" class="form-control input-sm checkboxes" value="" name="checkitem" id="checkitem"></input> '+
                '</td>'+                        
                '<td style="width: 5%">'+           
                    ...
                    '</button>'+
                '</td>'+
                '<td>'+val.date+'<input type="hidden" id="hd_date_inspected" value="'+val.date+'" name="hd_date_inspected[]"></td>'+
                '<td>'+val.po_no+'<input type="hidden" id="hd_pono" value="'+val.po_no+'" name="hd_pono[]"></td>'+
                '<td>'+val.device_name+'<input type="hidden" id="hd_device_name" value="'+val.device_name+'" name="hd_device_name[]"></td>'+
                '<td>'+qty+'<input type="hidden" id="hd_qty" value="'+qty+'" name="hd_qty[]"></td>'+
                '<td>'+val.total_num_of_lots+'<input type="hidden" id="hd_total_lots" value="'+val.total_num_of_lots+'" name="hd_total_lots[]"></td>'+
            '</tr>';    
