								   									 	...
								   									 	<?php if(isset($mk_data_details)): ?>
									   									 	<?php foreach($mk_data_details as $mkddata): ?>
										   									 	<tr id="details<?php echo e($cnt); ?>">
										   									 		...
										   									 		<td style="width:7.6%"><?php echo e($mkddata->location); ?></td>
										   									 		<td style="width:7.6%"><?php echo e($mkddata->drawing_no); ?></td>
										   									 		<td style="width:7.6%"><?php echo e($mkddata->supplier); ?></td>
										   									 		<td style="width:7.6%"><?php echo e($mkddata->whs100); ?></td>
										   									 		<td style="width:7.6%"><?php echo e($mkddata->whs102); ?></td>
										   									 	</tr>
									   									 		<?php $cnt++;?>
									   									 	<?php endforeach; ?>
								   									 	<?php endif; ?>
 	...
 	$( document ).ready(function(e) {
		...
		//barcode fired----------------------------------
 		$('#hd_barcode').keypress( function(e){
			...
			if(key == 13) {
				...
				$.ajax({
					...
					if (data == null) {
						$('#err_msg').html("Item Code is not available in this P.O.");
						$('#msg').modal('show');
						$('#hd_barcode').val('');
					} else {
 		...
 		$('#lotnoiss').change(function(){
			...
			$.ajax({
				...
				if(data == "NM"){
					$('#err_msg').html("Lot number not Match!");
 					$('#msg').modal('show');
				}else{
 		...
 		$('#btn_kittinglist').on('click', function() {
 			if ($('#issuanceCount').val() > 0) {
 				...
 				$('#err_msg').html("There are no Issuances for this P.O.");
 				$('#msg').modal('show');
 			}
 		});
				...
				if(data == '') {
					...
					$.each(data, function(i,det){
						...
						tblKitDetails = '<tr>'+
											...
											'</td>'+
											'<td style="width:7.6%">'+det.location+
											...
											'</td>'+
											'<td style="width:7.6%">'+det.whs100+
												'<input type="hidden" id="whs100kit" name="whs100kit" value="">'+
											'</td>'+
											'<td style="width:7.6%">'+det.whs102+
		...
		function transfertosakidashi(e) {
			...
			if (isCheck($('.kitdetailschk'))) {
				...
				$.ajax({
					...
					$('#err_msg').html("There is an error while transfering");
	 				$('#msg').modal('show');
				});
			} else {
				$('#err_msg').html("Please select an item.");
	 			$('#msg').modal('show');
			}
		...
		function delDetails() {
			...
			if (isCheck($('.kitdetailschk'))) {
				...
				$.ajax({
					...
					$('#err_msg').html("There is an error while deleting");
	 				$('#msg').modal('show');
				});
			} else {
				$('#err_msg').html("Please select an item.");
			...
			if ($('#tblfifoAdd tr').length > 0 || $('#tblfifoEdit tr').length > 0) {
				...
				tblIssDetails = '<tr id="'+newdetid+'">'+
									...
									'<td id="barcode-'+newdetid+'">'+
										'<a href="javascript:;" class="btn input-sm grey-gallery barcodebtn" data-detailid="'+newdetid+'" data-item="'+code+'" data-item_desc="'+name+'" data-kit_qty="'+kitqty+'" data-issued_qty="'+issuedqty+'" data-lot_no="'+lotno+'" data-location="'+location+'">'+
											'<i class="fa fa-barcode"></i>'+
										'</a>'+
									'</td>'+
								'</tr>';
		...
		$('#tblfifoAdd').on('click', '.btn_select_lot', function() {
			if ($(this).attr('data-rowcount') != 1) {
				$('#msg').modal('show');
	            $('#title').html('<strong><i class="fa fa-exclamation-triangle"></i></strong> Warning!')
	            $('#err_msg').html('FIFO is Recommended.');
			} else {
				var lotno = $(this).attr('data-lotno');
				$('#lotnoiss').val(lotno);
		...
		$('#tblIssDetails').on('click', '.barcodebtn', function() {
			...
			var template = '<table class="table" style="font-size: 14px; font-weight: 900;">'+
								'<tr>'+
									'<td class="col-xs-12" colspan="3">'+
										'<svg class="col-xs-12" id="po_br"></svg>'+
									'</td>'+
								'</tr>'+
								'<tr>'+
									'<td class="col-xs-2">Qty.</td>'+
									'<td class="col-xs-4 text-center" id="qty_br">'+issued_qty+'</td>'+
									'<td class="col-xs-6">pc(s)</td>'+
								'</tr>'+
								'<tr>'+
									'<td class="col-xs-2">LOT.</td>'+
									'<td class="col-xs-10 text-center" text-center colspan="2" id="lotno_br">'+lot_no+'</td>'+
								'</tr>'+
								'<tr>'+
									'<td class="col-xs-12 text-center" colspan="3" id="item_desc_br">'+item_desc+'</td>'+
								'</tr>'+
								'<tr>'+
									'<td class="col-xs-12" colspan="3">'+
										'<svg class="col-xs-12" id="item_br"></svg>'+
									'</td>'+
								'</tr>'+
							'</table>';
			// JsBarcode("#po_br", po, {width: 3,height: 25})+
			// JsBarcode("#item_br", item, {width: 3,height: 30})+

			if (isOnMobile() == true) {
				printBRcode(id,item,item_desc,kit_qty,issued_qty,lot_no,location,po);
			} else {
				printBRcode(id,item,item_desc,kit_qty,issued_qty,lot_no,location,po);
				$('#err_msg').html("Please use mobile device.");
	 			$('#msg').modal('show');
			}
		});

		function printBRcode(id,item,item_desc,kit_qty,issued_qty,lot_no,location,po) {
			var token = "<?php echo e(Session::token()); ?>";
			var data = {
				_token: token,
				id:id,
			...
			window.location.href="/wbskit-brprint?id="+id;//+id+"&item="+item+"&item_desc="+item_desc+"&kit_qty="+kit_qty+"&issued_qty="+issued_qty+"&lot_no="+lot_no+"&location="+location+"&po="+po
            ...
            data: data,
     		...
     		$.each(data, function(i, x) {
     			...
     			tblfifoAdd = '<tr>'+
                                ...
                                '<td style="width:16.6%">'+x.created_at+'</td>'+
                            '</tr>';
